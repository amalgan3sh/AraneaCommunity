...
<section class="sign-in-page">
   <div class="container-fluid">
            ...
            <div class="sign-in-from">
               ...
               <form class="mt-5">
                  <div class="form-group text-start mb-0">
                     ...
                     <input type="email" class="form-control mb-0" placeholder="Your E-mail" id="resetEmail">
                  </div>
                  <button type="button" class="btn btn-primary mt-4 fw-semibold text-uppercase letter-spacing-1" onclick="sentresetmail()">reset password</button>
               </form>
            </div>
         </div>
      </div>
    ...
    $.ajax({
        url: '/User/sentresetmail',  // Adjust the URL according to your route/controller
        type: 'POST',
        data: { email: email },
        success: function(response) {
            var result = JSON.parse(response);
            alert(result.message);  // Display the success or failure message
        },
        error: function(xhr, status, error) {
            alert('Error: Unable to send reset email.');
